...
class SearchJob(Job):
    ...
    def create(config, dataset, parent_job=None):
        ...
        if config.get("search.type") == "manual":
            from kge.job import search_manual

            return search_manual(config, dataset, parent_job)
        elif config.get("search.type") == "grid":
            from kge.job import search_grid

            return search_grid(config, dataset, parent_job)
        elif config.get("search.type") == "ax":
            from kge.job import search_ax

            return search_ax(config, dataset, parent_job)
        else:
